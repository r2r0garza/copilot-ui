...
import type { TaskRecord, TaskSnapshotPersistence } from "../../features/tasks";
...
export class WorkspaceStore implements TaskSnapshotPersistence {
  ...
  public loadTaskSnapshots(): readonly TaskRecord[] {
    const rows = this.db.prepare(
      "SELECT state_json AS stateJson FROM task_runtime_snapshots WHERE (task_id, version) IN (SELECT task_id, MAX(version) FROM task_runtime_snapshots GROUP BY task_id) ORDER BY task_id",
    ).all() as { stateJson: string }[];
    return rows.map((row) => JSON.parse(row.stateJson) as TaskRecord);
  }
  public saveTaskSnapshot(task: TaskRecord, eventName: string, at: string): void {
    this.transaction(() => {
      const current = this.db.prepare("SELECT COALESCE(MAX(version), 0) AS version FROM task_runtime_snapshots WHERE task_id = ?")
        .get(task.taskId) as { version: number };
      this.db.prepare(
        "INSERT INTO task_runtime_snapshots (task_id, version, state_json, event_name, created_at) VALUES (?, ?, ?, ?, ?)",
      ).run(task.taskId, current.version + 1, JSON.stringify(task), eventName, at);
      this.appendEvent(eventName, task.taskId, JSON.stringify({
        taskState: task.state,
        contractVersion: task.confirmedContractVersion,
        graphRevision: task.graphRevisions.length,
      }), at);
    });
  }
  ...
  private migrate(): void {
    ...
    this.db.exec("CREATE TABLE IF NOT EXISTS task_runtime_snapshots (task_id TEXT NOT NULL, version INTEGER NOT NULL CHECK(version >= 1), state_json TEXT NOT NULL, event_name TEXT NOT NULL, created_at TEXT NOT NULL, PRIMARY KEY(task_id, version)); CREATE TRIGGER IF NOT EXISTS task_runtime_snapshot_no_update BEFORE UPDATE ON task_runtime_snapshots BEGIN SELECT RAISE(ABORT, 'task-runtime-snapshot-append-only'); END; CREATE TRIGGER IF NOT EXISTS task_runtime_snapshot_no_delete BEFORE DELETE ON task_runtime_snapshots BEGIN SELECT RAISE(ABORT, 'task-runtime-snapshot-append-only'); END;");
