...
async function generateChatTitle(model: vscode.LanguageModelChat, firstMessage: string): Promise<string> { try { const prompt = `Write a concise title of 5 to 7 words for a conversation whose first user message is below. Return only the title, no punctuation, labels, quotes, or explanation.\n\n${firstMessage}`; const response = await model.sendRequest([vscode.LanguageModelChatMessage.User(prompt)], {}); let title = ""; for await (const fragment of response.text) title += fragment; const words = title.replace(/["'`*_#.:!?]/g, "").replace(/\s+/g, " ").trim().split(" ").filter(Boolean).slice(0, 7); if (words.length >= 5) return words.join(" "); } catch { /* Keep a useful local fallback when a provider cannot serve the second request. */ } return fallbackChatTitle(firstMessage); }
