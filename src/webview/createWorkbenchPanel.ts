...
<html lang="en">
  ...
  <body>
    ...
    <script nonce="${nonce}">
      ...
      document.addEventListener('click', (event) => { const control = event.target.closest('[data-chat-action]'); if (!control) return; const action = control.dataset.chatAction; if (action === 'delete' && !confirm('Permanently delete this trashed Chat? This cannot be undone.')) return; const selectedTitle = document.querySelector('.session-item[aria-current="true"] span')?.textContent || ''; const title = action === 'rename' ? prompt('Rename this Chat', selectedTitle) : undefined; if (action === 'rename' && title === null) return; vscode.postMessage({ type: 'chat-action', action, chatId: control.dataset.chatId, title }); });
